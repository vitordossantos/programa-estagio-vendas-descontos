      ...
      unsigned addToShoppingCart(unsigned productID,Array & ar)
        {
           ...
           // parte do codigo que verifica se o codigo do produto representa um produto existente e que faz adicao 
            // cout<<"Veja firstProduct "<<firstProduct<<endl;
...
// Fim da parte do codigo que implementa 2 funcoes de uso geral que nao estao implementadas dentro de classes
 ...
 TEST(addToShoppingCart)
  ...
  //  cout<<"Veja v na outra funcao "<<&v<<endl;
    delete shoppingCart;
  // cout<<"Veja shoppingCart na outra funcao1 "<<shoppingCart<<endl;
 ...
 while(j<NUMBER_VALIDATE_PRODUCT_ID_TESTS)
  {
      ...
      while(i< numberOfCharactersString)
       ...
       while(i<numberOfCharactersString && !testFunctionStringFalseResult)
        ...
        if(shoppingCart->validateProductID(productID) == testFunctionStringFalseResult)
                     CHECK(false);
        
  j++;
  }

  //cout<<"Veja shoppingCart na outra funcao "<<shoppingCart<<endl;
  delete shoppingCart;
 // cout<<"Veja shoppingCart na outra funcao2 "<<shoppingCart<<endl;
...
/*TEST(setDiscount)

 {
  ...
  string couponCode;
  Shoppingcart* shoppingCart = new Shoppingcart;
  cout<<"Veja shoppingCart "<<shoppingCart<<endl;
  Value v;
  Value v2;
  Array ar;
  Array ar2;
               ...
               if(ar2[i]["type"].getString() == "percentage")
                  {
                   ...
                   cout<<"Veja shoppingCart->getDiscount() e discount "<<setprecision(2)<<fixed<<shoppingCart->getDiscount()<<" "<<setprecision(2)<<fixed<<discount<<endl;
